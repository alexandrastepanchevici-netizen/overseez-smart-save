...
export default function ReviewSection() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('user_id', user.id)
      .then(({ data }) => {
        if (data && (data as any[]).length > 0) setSubmitted(true);
      });
  }, [user]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error } = await supabase
        .from('reviews' as any)
        .upsert({ user_id: user.id, rating, feedback } as any, { onConflict: 'user_id' });
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto text-center py-6">
        ...
        <p className="font-display font-semibold text-lg mb-1">Thanks for your feedback!</p>
        <p className="text-sm text-muted-foreground">Your review helps us improve Overseez.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-lg mx-auto bg-card border border-border rounded-xl p-6 sm:p-8">
      <h3 className="font-display font-semibold text-lg mb-1 text-center">Rate your experience</h3>
